...
export default class UploaderRtcClient {
    ...
    private async asnwser(sender: string, sessionDescription: any) {
        try {
            ...
            await this.firebaseSignallingClient.sendAnswer("uploader", this.rctPeerConnection.localDescription as RTCSessionDescription);
        } catch (e) {
            throw e;
        }
    }
    ...
    private async offer() {
        try {
            ...
            await this.firebaseSignallingClient.sendOffer(this.rctPeerConnection.localDescription as RTCSessionDescription);
        } catch (e) {
            throw e;
        }
    }
    ...
    private setOnicecandidateCallback() {
        this.rctPeerConnection.onicecandidate = (async (event: RTCPeerConnectionIceEvent) => {
            if (event.candidate) {
                await this.firebaseSignallingClient.sendCandidate("uploader", event.candidate.toJSON());
            }
        });
    }
